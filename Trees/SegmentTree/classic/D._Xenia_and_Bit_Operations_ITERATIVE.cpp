...
* Memory Limit: 256
* Time Limit: 2000
* Start: Sáb 06 abr 2024 22:00:43    
* */
...
//APLICAMOS SEGMENT TREE ITERATIVO CON BITWISE OPERATIONS  ---> DIOJ MIO ES HERMOSO :'V.

const int N=1<<16;
int n,m,I,VA,T[N<<2];;
 inline void modify(bool ok,int i){
      ok ? T[i] = (T[i<<1] | T[i<<1|1]) : T[i]= (T[i<<1] ^ T[i<<1|1]);
 }
 inline void init(){
     bool ok=1;
     for (int i=n-1;i;--i){
         modify(ok,i);
         if((i&(i-1))==0) ok^=1;  // XOR en cuyo caso I sea potencia de 2.
     }
 }
 inline int upt(int I,int VA){
     I=I+n-1;
     T[I]=VA;
     I>>=1;
     bool ok=1;
